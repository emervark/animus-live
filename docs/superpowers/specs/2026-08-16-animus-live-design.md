...
> **M0-2 correction (2026-08-17):** the rounding, the `scale_factor()` multiply and the ≥ 1 clamp are all right. The **debounce is not** — on the hardware tested it buys nothing and costs a visible artifact. Ten seconds of rapid splitter dragging produced **zero** wgpu validation errors and zero warnings both with it and without it; what the debounce does produce is a rubber-band stretch, because the render target deliberately is not rebuilt while the size is unsettled. Asked to compare, the user preferred the undebounced version outright. **M1 resizes immediately.** Caveat: one GPU, one driver, and the resize triggers that matter at a venue — a per-monitor DPI change and entering fullscreen — remain untested.
...
> **M0-2 correction (2026-08-17): do not do this — it disables the input it appears to protect.** In egui 0.34 `wants_pointer_input()` expands to `egui_is_using_pointer() || (is_pointer_over_egui() && !any_down())`, and the viewport *is* an egui widget, so the pointer is over an egui area whenever it is over the viewport. Hovering to scroll-zoom (no button down) and clicking (reported on release, no button down) both evaluate to "egui wants the pointer" and are dropped; middle-drag survives only because a held button makes the same expression false. That guard is for applications whose 3D world is drawn *outside* egui. Here the correct gates are `response.hovered()` and `response.clicked()`, which already account for occlusion — **and the image must be created with `.sense(Sense::click_and_drag())`, since `egui::Image` defaults to `Sense::hover()` and otherwise never reports a click or a drag at all.** See `docs/spikes/m0-2-egui-viewport.md`.
